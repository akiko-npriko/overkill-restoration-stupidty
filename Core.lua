...
function restoration:akiko_randomize_unique_units()
	if Network:is_server() then
		for key,setting in pairs(akiko_default_unique_units_keys) do
			local blahmreowp = key[tweak_data.levels:get_ai_group_type()] or key.default -- fix ai group type later
...
restoration._sound_move_dt = nil
	
function restoration:sound_Is_ply_OK()
	if not managers.groupai:state():whisper_mode() or not managers.navigation or not managers.navigation:is_data_ready() then
		return
	end
	if not restoration.sound_move_data or not restoration.sound_move_data.ply or not restoration.sound_move_data.ply._unit or not alive(restoration.sound_move_data.ply._unit) then
		return
	end
	return true
end

function restoration:sound_create_point(sound_source_pos)
	...
	if type(playerss) == "table" then
		...
		if cc and cc.unit and alive(cc.unit) then
			restoration.sound_move_data.ply = cc.unit:base()	
		end
	end
	if not restoration:sound_Is_ply_OK() then
		return
	end
	local ply = restoration.sound_move_data.ply
	if not u_key then
		u_key = tostring(ply._unit:key())
	end
	local __now_t = TimerManager:game():time()
	if restoration._sound_move_dt and restoration._sound_move_dt > __now_t then
		return
	end
	restoration._sound_move_dt = __now_t + 3
