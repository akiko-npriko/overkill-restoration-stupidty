Hooks:PostHook(WeaponFactoryTweakData, "init", "byakurenhijiriyes", function(self)
	...
	if not BeardLib.Utils:FindMod("RestorationMod") then
			...
			for k, used_part_id in ipairs(self.wpn_fps_ass_amcar.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_amcar.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_amcar_body_ddm4.forbids, used_part_id) then
						if self.parts[used_part_id].type == "lower_reciever" or
		...
		if self.parts.wpn_fps_ass_m16_body_mcbravo then
			for k, used_part_id in ipairs(self.wpn_fps_ass_m16.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_m16.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_m16_body_mcbravo.forbids, used_part_id) then
						if self.parts[used_part_id].type == "upper_reciever" or
						self.parts[used_part_id].type == "lower_reciever" or
						self.parts[used_part_id].type == "grip" or
						self.parts[used_part_id].type == "foregrip" or
		...
		if self.parts.wpn_fps_ass_ak_body_creedmoor then
			for k, used_part_id in ipairs(self.wpn_fps_ass_74.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_74.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_ak_body_creedmoor.forbids, used_part_id) then
						if self.parts[used_part_id].type == "upper_reciever" or
						self.parts[used_part_id].type == "lower_reciever" or
						self.parts[used_part_id].type == "grip" or
						self.parts[used_part_id].type == "foregrip" or
		...
		if self.parts.wpn_fps_ass_scar_body_hamr then
			for k, used_part_id in ipairs(self.wpn_fps_ass_scar.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_scar.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_scar_body_hamr.forbids, used_part_id) then
						if self.parts[used_part_id].type == "upper_reciever" or
						self.parts[used_part_id].type == "lower_reciever" or
						self.parts[used_part_id].type == "grip" or
						self.parts[used_part_id].type == "magazine" or
		...
		if self.parts.wpn_fps_ass_g3_body_hk33 then
			for k, used_part_id in ipairs(self.wpn_fps_ass_g3.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_g3.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_g3_body_hk33.forbids, used_part_id) then
						if self.parts[used_part_id].type == "magazine" or
						self.parts[used_part_id].type == "barrel" then
							table.insert(self.parts.wpn_fps_ass_g3_body_hk33.forbids, used_part_id)
						end
					end
				end
			end
		...
		if self.parts.wpn_fps_smg_mac10_body_m4_v2 then
			for k, used_part_id in ipairs(self.wpn_fps_smg_mac10.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_smg_mac10.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_smg_mac10_body_m4_v2.forbids, used_part_id) then
						if self.parts[used_part_id].type == "stock" then
							table.insert(self.parts.wpn_fps_smg_mac10_body_m4_v2.forbids, used_part_id)
						end
					end
				end
			end
		...
		if self.parts.wpn_fps_smg_sterling_body_pdw then
			for k, used_part_id in ipairs(self.wpn_fps_smg_sterling.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_smg_sterling.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_smg_sterling_body_pdw.forbids, used_part_id) then
						if self.parts[used_part_id].type == "barrel" or
						self.parts[used_part_id].type == "magazine" or
						self.parts[used_part_id].type == "barrel_ext" then
							table.insert(self.parts.wpn_fps_smg_sterling_body_pdw.forbids, used_part_id)
						end
					end
				end
			end
		...
		if self.parts.wpn_fps_ass_akm_body_tacticool then
			for k, used_part_id in ipairs(self.wpn_fps_ass_akm.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_akm.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_akm_body_tacticool.forbids, used_part_id) then
						if self.parts[used_part_id].type == "barrel" or
						self.parts[used_part_id].type == "upper_reciever" or
						self.parts[used_part_id].type == "foregrip" or
						self.parts[used_part_id].type == "grip" then
							table.insert(self.parts.wpn_fps_ass_akm_body_tacticool.forbids, used_part_id)
						end
					end
				end
			end
		...
		if self.parts.wpn_fps_ass_contraband_body_little then
			for k, used_part_id in ipairs(self.wpn_fps_ass_contraband.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_ass_contraband.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_ass_contraband_body_little.forbids, used_part_id) then
						if self.parts[used_part_id].type == "magazine" then
							table.insert(self.parts.wpn_fps_ass_contraband_body_little.forbids, used_part_id)
						end
					end
				end
			end
	...
	--[[if self.parts.wpn_fps_smg_polymer_body_nine then
			for k, used_part_id in ipairs(self.wpn_fps_smg_polymer.uses_parts) do
				if self.parts[used_part_id] and self.parts[used_part_id].type then
					if not table.contains(self.wpn_fps_smg_polymer.default_blueprint, used_part_id) and not table.contains(self.parts.wpn_fps_smg_polymer_body_nine.forbids, used_part_id) then
						if self.parts[used_part_id].type == "magazine" then
							table.insert(self.parts.wpn_fps_smg_polymer_body_nine.forbids, used_part_id)
						end
					end
				end
			end
