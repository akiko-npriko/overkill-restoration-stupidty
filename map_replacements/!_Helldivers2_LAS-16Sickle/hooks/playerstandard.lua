...
Hooks:PreHook(PlayerStandard, "update", "pewpewupdate", function(self, t, dt)
	...
	if primary and primary._sweetliberty then
		self:_primary_pewpew_ammo(t, dt)
	end
end)

function PlayerStandard:_primary_pewpew_ammo(t, dt)
	...
	if primary then
		local regen_ammo_time = primary._sweetliberty.regen_ammo_time or 0.5
		local regen_rate = primary._sweetliberty.regen_rate or 10
		...
		if primary._primary_regenerate_ammo_timer and not primary:clip_empty() and ((active and not self:_is_reloading()) or (not active)) then
			...
			if primary._primary_regenerate_ammo_timer < 0 then
				self:primary_pewpew_ammo(dt * primary._primary_regen_rate)
...
function PlayerStandard:primary_pewpew_ammo(value)
	local primary = self._unit:inventory():unit_by_selection(2):base()
