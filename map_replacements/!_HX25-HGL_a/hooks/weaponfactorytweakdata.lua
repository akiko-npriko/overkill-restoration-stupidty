Hooks:PostHook(WeaponFactoryTweakData, "init", "HX25ModInit", function(self)
	local custom_wpn_id = "wpn_fps_gre_hx25"
	local stance_wpn_id = "wpn_fps_pis_lemming"
	for _, part_id in pairs(self[custom_wpn_id].uses_parts) do
		if self.parts[part_id] then
			if self.parts[part_id].type == "sight" or self.parts[part_id].a_obj == "a_rds" then
