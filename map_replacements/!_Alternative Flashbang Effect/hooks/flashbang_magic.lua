-- All code by fuglore (fugsystem)
-- I miss Hyper Heisting :^(
...
Hooks:PostHook(CoreEnvironmentControllerManager, "update", "update_hh_flashbang", function(self, t, dt)	
	if self._concussion_effect and self._current_concussion <= 0.8 then
		if self._pdth_conc then
			...
			World:effect_manager():set_simulator_var_float(self._concussion_effect, blindness_ids, opacity_ids, opacity_ids, value)
...
Hooks:OverrideFunction(CoreEnvironmentControllerManager, "set_flashbang", function(self, flashbang_pos, line_of_sight, travel_dis, linear_dis, duration, no_offset, no_effect)	
	...
	if self._current_concussion > 1 and not self._concussion_effect then
		...
		if BLT.Mods:GetModByName("PDTH Contours") then
			...
			self._concussion_effect = World:effect_manager():spawn({
				effect = Idstring("effects/pd2_mod_hh_flashbang/conc_floaties_pdth"),
				position = Vector3(),
				rotation = Rotation()
			})
		else
			self._concussion_effect = World:effect_manager():spawn({
				effect = Idstring("effects/pd2_mod_hh_flashbang/conc_floaties"),
				position = Vector3(),
				rotation = Rotation()
			})
		end
