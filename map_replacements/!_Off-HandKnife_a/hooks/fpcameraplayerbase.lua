...
function FPCameraPlayerBase:anim_clbk_spawn_knife_shell()
	--log("anim_clbk_spawn_knife_shell called")
...
function FPCameraPlayerBase:_unspawn_knife_shell()
	--log("_unspawn_knife_shell called")
	if not _knife_shell or not alive(_knife_shell) then
		return
	end
...
Hooks:PostHook(FPCameraPlayerBase, "destroy", "destroy_offhand_knife", function(self)
    --log("destroyknife called")
	self._unspawn_knife_shell()
end)