...
function GroupAIStateBase:_determine_objective_for_criminal_AI(unit, ...)
	...
	elseif movement._should_stay and movement._should_stay_pos then --Hopipp Useful Bots Stuff Below :3
		return {
			type = "defend_area",
			scan = true,
	...
	elseif alive(logic_data._latest_follow_unit) then
		return {
			type = "follow",
			scan = true,
			is_default = true,
			follow_unit = logic_data._latest_follow_unit
		}
	end
