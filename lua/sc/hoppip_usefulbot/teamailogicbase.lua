...
Hooks:PostHook(TeamAILogicBase, "_set_attention_obj", "_set_attention_obj_ub", function (data, att, react)
	...
	-- mark
	if restoration.usefulbot_settings.mark_specials and (not data._next_mark_t or data._next_mark_t < data.t) then
...
function TeamAILogicBase._check_deliver_bag(data)
	if data.cool or Monkeepers or not restoration.usefulbot_settings.secure_loot then
		return
	end
