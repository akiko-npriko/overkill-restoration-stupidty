...
Hooks:PostHook(TeamAIDamage, "_apply_damage", "_apply_damage_ub", function (self)
	...
	if restoration.usefulbot_settings.announce_low_hp and (not self._said_hurt_t or self._said_hurt_t + 10 < t) and self._health_ratio < 0.3 and not self:need_revive() and not self._unit:sound():speaking() then
		self._said_hurt_t = t
