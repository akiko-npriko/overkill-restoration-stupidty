...
function TeamAILogicIdle.is_valid_intimidation_target(unit, unit_tweak, unit_anim, unit_damage, data, distance)
	if restoration.usefulbot_settings.dominate_enemies > 2 then
		return false
	end
	...
	if restoration.usefulbot_settings.dominate_enemies > 1 then
...
function TeamAILogicIdle._get_priority_attention(data, attention_objects, reaction_func, ...)
	local ub_priority = restoration.usefulbot_settings.targeting_priority
...
function TeamAILogicIdle.on_long_dis_interacted(data, other_unit, secondary, ...)
	...
	if objective_type == "revive" and had_bag and move_speed_modifier > 1 - restoration.usefulbot_settings.drop_bag_percentage and not movement:carrying_bag() then
