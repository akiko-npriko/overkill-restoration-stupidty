...
function PlayerDamage:_calc_armor_damage(attack_data)
	...
	if self:get_real_armor() > 0 then
		...
		if pm:has_category_upgrade("player", "adaptive_plate_multiplier") then
			...
			local tramaapstat = s+1
			if tramaapstat > 0 then
				local debugshit = pm.akiko_tramadamage_ap[tramaapstat] + (attack_data.damage * (1/10))
				pm.akiko_tramadamage_ap[tramaapstat] = math.floor(pm.akiko_tramadamage_ap[tramaapstat] + (attack_data.damage * (1/10)))
...
function PlayerDamage:get_adaptive_plate_stage_count()
	...
	throwables = managers.player:upgrade_value("player", "armor_plate_bag_amount", 6)
	...
	if tweak_data.blackmarket.projectiles.adaptive_plate.max_amount ~= throwables then
		...
		for i=1,throwables,1 do
			DelayedCalls:Add("refresh_apc_inst", 0.25 * i, function()
				pm:speed_up_grenade_cooldown((tweak_data.blackmarket.projectiles[managers.blackmarket:equipped_grenade()].base_cooldown))
			end)
		end
